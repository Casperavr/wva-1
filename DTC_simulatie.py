...
#Fuel Properties
LHV_diesel = 42700 #[kJ/kg]  Lower Heating Value (hoeveelheid energie die vrijkomt als 1kg brandstof wordt verbrand.)
LHV_ammonia = 18646 #[kJ/kg]  Lower Heating Value (hoeveelheid energie die vrijkomt als 1kg brandstof wordt verbrand.)
print('fuel properties loaded')

#Physical constant
rho_sea = 1026 #[kg/m^3] density of sea water
mu_seawater = 1.1892e-6 #[m^2/s] dynamic viscosity of water
print('water properties loaded')

#Ship Data
w = 0.275 # [-] wake factor uit HM sheet Jasper
c1 = 5.72 #6.182 #[-] resistance coefficient of R = c1 * (v_s ^ 2)
thrust_factor = 0.09 #[-] thrust deduction factor uit HM sheet Jasper
m_ship = 8000000 #[kg] ship mass
l_ship = 355 #[m] ship length (lwl)
nabla_ship = 173467 #[m^3] underwater volume
s_ship = 22032 #[m^2] wet surface
cb_ship = 0.661 #[-] block coefficient
k_ship = 0.145 #[-] form factor
v_design_ship = 25 * 0.5144 #[m/s] design speed
...
plt.grid(True)
plt.tight_layout()
plt.savefig(f'figures_dtc/start_fig01_EXPERIMENT-{EXPERIMENT}.jpg')
...
plt.savefig(f'figures_dtc/Speeds-Time_EXPERIMENT-{EXPERIMENT}.jpg')
...
def plotLineGraph(x, ylist, xlabel, ylabels, title, yunit):
    ...
    plt.ylabel(yunit)
    plt.grid(True)
    plt.savefig(f'figures_dtc/{title.replace(" ", "")}_EXPERIMENT-{EXPERIMENT}.jpg')    
